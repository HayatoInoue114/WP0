...
void Blast::Initialize(Player* player, Vector3 position, int level) {
	...
	model_.CreateFromObj("Blast", "Blast");
	isAlive_ = true;
	isFeed_ = false;
	velocityXZ_ = 0;
...
void Blast::OnCollisionWithPlayerBullet(PlayerBullet* bullet)
{
	if (!isFeed_) {
		Log("Hit!!\n");
...
void Blast::CalculateDetectionRange() {
	//範囲内か計算
	distanceToPlayer_ = CalculateEuclideanDistance(player_->GetPosition(), worldTransform_.translate());
...
void Blast::Chace() {
	if (isTrace_ && !isBlast_) {
		...
		Vector3 toPlayer = player_->GetPosition() - worldTransform_.translate();
...
void Blast::FeedMove()
{
	if (ToPlayerDis() <= param.searchDistance) {
...
float Blast::ToPlayerDis() {
	return (float)sqrt(pow(player_->GetPosition().x - worldTransform_.translate().x, 2) + pow(player_->GetPosition().z - worldTransform_.translate().z, 2));
}
...
Vector3 Blast::Jump(const Vector3& initialPosition, float jumpHeight, float jumpDuration, float currentTime) {
	...
	worldTransform_.rotate().y = CalculateAngle(worldTransform_.translate(), player_->GetPosition());
